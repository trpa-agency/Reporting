...
def run(df_csv: pd.DataFrame, csv_lookup: dict) -> dict:
    ...
    for year in sorted(CSV_YEARS):
        ...
        # Filter centroid layer to only this year's missing APNs
        apn_filter = " OR ".join(
            f"CSV_APN = {chr(39)}{a}{chr(39)}" for a in yr_missing)
        ...
        # Pass 2: CLOSEST for still-unmatched
        matched_p1 = {r["CSV_APN"] for r in crosswalk_rows if r["Year"] == year}
        still_missing = yr_missing - matched_p1
        if still_missing:
            apn_filter2 = " OR ".join(
                f"CSV_APN = {chr(39)}{a}{chr(39)}" for a in still_missing)
