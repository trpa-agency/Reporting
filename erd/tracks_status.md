...
> **Audience: TRPA dev team, Dan, Ken, anyone reading or extending the pipeline.**
...
| File | Source | Rows |
|---|---|---:|
| `apn_genealogy_master.csv` | Manual master (Ken-curated) | ~5K |
...
Per Dan's framing (5/11/2026), organized by track:
...
Today the dashboards bypass the schema layer and read CSV / JSON exports from Ken's xlsx files (via converter scripts in `parcel_development_history_etl/scripts/`). Once `vCommodityLedger` lands as a real DB view, dashboards switch to direct query.
...
When Ken sends a refreshed xlsx, drop in `data/raw_data/` and run the matching converter (see `html/index.md` for commands).
...
Track C captures every QA adjustment to existing-development quantities with rationale, so Ken's Excel-only audit trail becomes a queryable structured store.

### What this track does

Ken's master record lives at [`data/qa_data/CA Changes breakdown.xlsx`](../data/qa_data/) — 44,371 row-cycles of per-APN change rationale across the **2023 and 2026 big-sweep campaigns**. Track C does three things:
...
| `qa_reconciliation_status.csv` | 218,192 | One row per s06 finding, labeled by Ken's address status. |
| `qa_ken_only_corrections.csv` | 1,035 | Ken's events with no matching s06 finding — pre-detection corrections, s06 false negatives, or out-of-scope. |
...
1. Drop the new file at `from_ken/CA Changes breakdown.xlsx` (canonical) or `data/qa_data/CA Changes breakdown.xlsx` (working).
...
- **C.O1** — 30% canonical-vocab match rate. Recommend a `qa_data/correction_category_mapping.csv` to preserve Sheet2 as authoritative while mapping Ken's Sheet1 paraphrases.
- **C.O2** — Year-aware reconciliation matching: a Ken cycle-Y correction can only address an s06 finding for Year X if X ≤ Y.
- **C.O3** — Compute `PreviousQuantity` / `NewQuantity` (Ken's XLSX carries deltas only). Join to `FINAL RES SUMMARY 2012 to 2025.xlsx` Residential sheet by canonical APN.
- **C.O4** — Resolve commodity per row (currently `SFRUU_or_MFRUU_TBD` because Ken's data doesn't split SFRUU/MFRUU/ADU).
