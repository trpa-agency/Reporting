...
## Findings from Ken's April 2026 deliverables (added 2026-04-30)

Ken sent the 2026 Cumulative Accounting Report and 5 supporting XLSX files
(at [`from_ken/`](../from_ken/) outside this worktree) on 2026-04-30. Reading
them resolved several open gaps in this trace and surfaced four new ones.
Detail in the gap sections below; summary here:

| Gap | Status after Ken's data |
|---|---|
...
[Ken's transactions XLSX](../data/raw_data/) and is published at
...
table, Plotly stacked bar per (jurisdiction × commodity)). Audience: Dan +
...
| Capacity ceiling | **MISSING** | `MaxRegionalCapacity` | **Gap**. Realized notebook leaves `MaxCapacity` null on `account.csv` ("until Ken confirms the authoritative source per commodity"). The dashboard *needs* this to show "% of capacity used." |
...
| `MaxRegionalCapacity` | **No source yet.** | Per Q3 in build notebook: "until Ken confirms the authoritative source per commodity." Likely lives on `dbo.CommodityPool` (one of the capacity columns) or in a TRPA Code reference table. |
...
- **G1.2 — `MaxRegionalCapacity` source.** ✅ **Resolved by Ken's 2026
...
- **G2.1 — `InitialCapacity` source.** ✅ **Resolved by Ken's
  `Additional Development as of April2026.xlsx` "LT Info Pools Balances"
  sheet + PPTX slide 5.** Per-pool starting capacities are in Ken's
...
- **G2.2 — CSLT pool-name granularity.** ✅ **Confirmed real.** Ken's
...
- **G2.7 — Unreleased allocations as a distinct category** (NEW from
  Ken's data). PPTX slide 5 names "UNRELEASED ALLOCATIONS = 770" as a
...
- **G2.8 — Bonus Units as a first-class movement type** (NEW from Ken's
...
> **Status of source code**: no HTML prototype, but Ken's
...
**Realized data found in Ken's deliverables (2026-04-30):**
...
3. **`CorrectionCategory` enum + `ReportingCycleYear` column on
   `ParcelDevelopmentChangeEvent`.** Both required to load Ken's CA
...
- **G3.5 — Project-level annotations** (NEW from Ken's data). Each
...
- **G3.6 — APN format normalization** (NEW from Ken's data). Many parcels
...
| ~~G1.2~~ | ~~`MaxRegionalCapacity` source~~ — ✅ **resolved**: model as `RegionalPlanCapacity(Commodity, PlanEra)` lookup; seed from Ken's PPTX slide 8 | `target_schema.md` §"reference entities" |
...
| ~~G2.1~~ | ~~`InitialCapacity` column on `PoolDrawdownYearly`~~ — ✅ **resolved**: seed from Ken's `LT Info Pools Balances` sheet, flag `Source='kens_xlsx_april_2026'` | `target_schema.md` §`PoolDrawdownYearly` |
...
| **G2.x TRPA Pool reconciliation** | Dashboard currently shows TRPA Pool = 144; Ken says correct value is 154. Don't blindly carry LT Info pool values; require Ken's manual corrections layer before display. | dashboard + ETL reconciliation, separate from schema |
...
1. **CSLT sub-pool resolution.** ✅ Resolved by Ken's data. Skip the
   `dbo.CommodityPool` query — Ken's pool reports confirm 3 distinct
   sub-pools.
...
3. **Initial-capacity column on `dbo.CommodityPool`.** ✅ Resolved by
   Ken's data. Skip the `SELECT TOP 1 *` — load initial capacities
   directly from Ken's `LT Info Pools Balances` sheet, flag source.
4. **Unreleased pool modeling.** Before filing G2.7, decide: pseudo-pool
   ...
   enum on `dbo.CommodityPool`. Talk through with Ken — this changes
...
6. **`Project` entity granularity.** Before filing G3.5, scope:
   ...
   Ken's narrative tracks at the report-card level — start there.
7. **APN canonicalization edge cases.** Before filing G3.6, verify the
   ...
   running uniqueness on `(canonical_apn, year)` against Ken's
   Residential sheet.
